    ...
    def _get_lightcurve_(self, p, obs, idx_orig=None):
        ...
        if obs is not None:
            ...
            if 'limmag_err' in self.plan.pointings.keys():
                ...
                limmag_err_ = []
                for i in range(len(obs['zp'])):
                    limmag_err_.append(np.float(self.plan.pointings['limmag_err'][np.where(self.plan.pointings['time']==lc['time'][i])[0]]))
...
class SurveyPlan( BaseObject ):
    ...
    def __init__(self, time=None, ra=None, dec=None, band=None, skynoise=None,
                 ...
                 load_opsim=None, limmag_err=None, **kwargs):
        """
        Parameters:
        ----------
        ...
        limmag_err: [np.array_like]   Standard deviation limiting magnitude, optional
        ...
        self.create(time=time,ra=ra,dec=dec,band=band,skynoise=skynoise,
                    ...
                    load_opsim=load_opsim, limmag_err=limmag_err, **kwargs)
